...
G4VPhysicalVolume* gneis::facility::Beam5::Construct() {
	...
	const auto solidC1Whole = new G4Tubs(strC1, 0.0, worldRadius, 200.0 * mm,
			0.0 * deg, 360.0 * deg);
	const auto solidC1Cut = new G4Box(strC1, 70.0 * mm, 90.0 * mm, 200.0 * m);
	...
	const auto solidC2Whole = new G4Tubs(strC2, 0.0, worldRadius, 355.0 * mm,
			0.0 * deg, 360.0 * deg);
	const auto solidC2Cut = new G4Box(strC2, 105.0 * mm, 50.0 * mm, 355.0 * m);
	...
	const auto solidC3 = new G4Tubs(strC3, 60.0 * mm, worldRadius, 300.0 * mm, 0.0 * deg, 360.0 * deg);
	const auto logicC3 = new G4LogicalVolume(solidC3,
			nist->FindOrBuildMaterial("G4_STEEL"), strC3);

	// Collimator C4
	const G4String strC4 = "collimator-C4";
	const auto solidC4 = new G4Tubs(strC4, 60.0 * mm, worldRadius, 0.5 * 875.0 * mm, 0.0 * deg, 360.0 * deg);
