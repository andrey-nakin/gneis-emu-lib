...
G4double gneis::facility::component::CollimatorC5::GetLength() {
	return length;
}

G4double gneis::facility::component::CollimatorC5::GetHalfLength() {
	return HalfOf(GetLength());
}

G4double gneis::facility::component::CollimatorC5::ToDouble(Diameter const d) {
	switch (d) {
	case Diameter::D50:
		return 50.0 * mm;
